...
SELECT p.first_name, p.last_name, p.salary, CHAR_LENGTH(first_name)
FROM players AS p
WHERE p.age >= 45;
...
WHERE c.id = (SELECT c.id FROM players_coaches GROUP BY c.id HAVING COUNT(*) > 1)
...
FROM players AS p
JOIN skills_data AS sa ON skills_data_id = sa.id
WHERE age < 23
AND position = 'A'
